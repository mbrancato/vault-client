...
public class VaultClient extends VaultClientMethods {
    ...
    private Map<String, VaultSecret> secrets = new HashMap<String, VaultSecret>();
    ...
    public String readKv(String name, String key) throws InterruptedException, IOException, AuthenticationException {
        // Assume kv-v2 mounted at /secret
        return readKv(name, key, 0, "/secret", 2);
    }

    public String readKv(String name, String key, int version, String mountPath, int kvVersion) throws InterruptedException, IOException, AuthenticationException {
        String path;
        String kvKey;
        if (kvVersion == 1) {
    ...
    public String read(String path, String key) throws IOException, InterruptedException, AuthenticationException {
    ...
    private JsonElement readJsonElement(String path, String key) throws IOException, InterruptedException, AuthenticationException  {
    ...
    private boolean loginGcp() throws IOException, InterruptedException {
        ...
        if (this.authRole != null) {
            googleUrlParams = String.format("audience=vault/%s&format=full", URLEncoder.encode(this.authRole, StandardCharsets.UTF_8.toString()));
        } else {
            googleUrlParams = "audience=vault&format=full";
        }

        googleMetadatUrl = String.format("http://metadata/computeMetadata/v1/instance/service-accounts/default/identity?%s", googleUrlParams);
    ...
    private boolean loginJwt(String jwt) throws IOException, InterruptedException {
        ...
        if (this.getDottedJsonPath("auth.client_token", loginResponse) != null) {
            ...
            if (this.getDottedJsonPath("auth.lease_duration", loginResponse) != null) {
                this.vaultTokenLeaseDuration = this.getDottedJsonPath("auth.lease_duration", loginResponse).getAsLong();
            }
            if (this.getDottedJsonPath("auth.policies", loginResponse) != null) {
                this.vaultPolicies = gson.fromJson(this.getDottedJsonPath("auth.policies", loginResponse), listOfStrings);
            }
            this.authenticated = true;
            return true;
        }
